...
// Here come the tables:
   ...
   unsigned int starts [] = 
   {
      0, 10, 13, 15, 17, 22, 25, 27, 31, 35, 38, 
      43, 46, 0 };

   unsigned int defaults [] = {
      0, 1, 0, 3, 4, 5, 3, 0, 7, 9, 10, 
      11, 0 };

   int parsetable [] = 
   {
      tkn_IDENTIFIER, 10001, 
      tkn_LEFT_PARANTHESES, 10002, 
      tkn_S, 10003, 
      tkn_T, 10004, 
      tkn_U, 10005, 
...
void reduction_0(
   ...
   std::list < token > :: iterator position,
   std::list < token > :: iterator S1,
   std::list < token > :: iterator PLUS_SIGN2,
   std::list < token > :: iterator T3 ) throw( refused )
{

#line 7 "simple.m"

{ reduce( stack, position, tkn_S, tkn_S ); return; }
...
void reduction_1(
   std::list < token > & stack,
   std::list < token > :: iterator position,
   std::list < token > :: iterator T1 ) throw( refused )
{
...
void reduction_2(
   ...
   std::list < token > :: iterator STAR_SIGN2,
   std::list < token > :: iterator U3 ) throw( refused )
{

#line 12 "simple.m"

{ reduce( stack, position, tkn_T, tkn_T ); return; }
...
void reduction_3(
   std::list < token > & stack,
   std::list < token > :: iterator position,
   std::list < token > :: iterator U1 ) throw( refused )
{
...
{ reduce( stack, position, tkn_U, tkn_U ); return; }
...
void reduction_5(
   std::list < token > & stack,
   std::list < token > :: iterator position,
   std::list < token > :: iterator LEFT_PARANTHESES1,
   std::list < token > :: iterator S2,
   std::list < token > :: iterator RIGHT_PARANTHESES3 ) throw( refused )
{
   ...
   while( true ) 
   {
      ...
      while( actions. size( ) > 1 ) 
      {
         ...
         try
         {
            ...
            {
case 0:
   reduction_0( parsestack, parsestack. end( ) - 3, parsestack. end( ) - 3, parsestack. end( ) - 2, parsestack. end( ) - 1 );
   break;
case 1:
   reduction_1( parsestack, parsestack. end( ) - 1, parsestack. end( ) - 1 );
   break;
case 2:
   reduction_2( parsestack, parsestack. end( ) - 3, parsestack. end( ) - 3, parsestack. end( ) - 2, parsestack. end( ) - 1 );
   break;
case 3:
   reduction_3( parsestack, parsestack. end( ) - 1, parsestack. end( ) - 1 );
   break;
case 4:
      ...
      else
      {
        ...
        std::cout << "error!\n"; 
                ...
                ignoredtokens < recoverlength )
         {
            ...
            {
               input. lookahead. push_front( token::token( tkn__recover ));
         ...
         states. push_back( actions. front( ));
         parsestack. push_back( token::token( tkn__recover ));
